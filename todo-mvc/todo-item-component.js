...
TodoItem.on("render", (state) => (
	<li className={classNames({completed: state.completed, editing: state.isEditing})}>
		<div className="view">
			<input className="toggle"
				type="checkbox"
				checked={state.completed}
				onChange={TodoItem.generate("CHANGE")} />
			<label onDoubleClick={TodoItem.generate("DOUBLE_CLICK")}>{state.text}</label>
...
TodoItem.on("CHANGE", (state, props) => TodoStore$.trigger("TOGGLE_COMPLETE", props.id));
TodoItem.on("DOUBLE_CLICK", () => TodoItemStore$.trigger("EDITING"));
TodoItem.on("CLICK", (state, props) => TodoStore$.trigger("DESTROY", props.id));
TodoItem.on("SAVE", (state, props, text) => TodoStore$.trigger("UPDATE", props.id, text))
